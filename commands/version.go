...
func NewVersionCommand(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "version",
		Short: "show knetctl version",
		RunE: func(cmd *cobra.Command, args []string) error {
			fmt.Printf("    version: %s\n", version)
			fmt.Printf("     commit: %s\n", commit)
			fmt.Printf("       date: %s\n", date)
			fmt.Printf("     source: %s\n", repoUrl)
			fmt.Printf(" rel. notes: https://docs.kubenet.dev/rn/%s\n", version)

			return nil
		},
	}

	cmd.AddCommand(NewVersionUpgradeCommand(ctx))
	return cmd
}

func NewVersionUpgradeCommand(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "upgrade knetctl to latest available version",
		//PreRunE: sudoCheck,
		RunE: func(cmd *cobra.Command, args []string) error {
			f, err := os.CreateTemp("", "knetctl")
